...
exec (stack, Fetch s:code,stored)= 
        let storedvalue = findStored stored s in
            (push storedvalue stack, code,(s,top stack):stored)
...
stack2Str stack 
    | isEmpty stack = ""
    | (top stack) == TT  = "True," ++ stack2Str (pop stack)
    | (top stack) == FF  = "False," ++ stack2Str (pop stack)
    | otherwise =
        let N n1 = top stack in
            (show n1) ++ "," ++ stack2Str (pop stack)
